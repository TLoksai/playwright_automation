...
(async () => {
  ...
  const headerLinks = await page.locator('header a').evaluateAll((nodes) => nodes.map((node) => ({html: node.outerHTML, text: node.textContent, href: node.getAttribute('href')})));
  console.log(JSON.stringify(headerLinks, null, 2));
  await browser.close();
})();
